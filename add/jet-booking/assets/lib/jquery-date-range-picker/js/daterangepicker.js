    ...
    $.fn.dateRangePicker = function(opt) {
        ...
        opt = $.extend(true, {
            ...
            hoveringTooltip: function(days, startTime, hoveringTime) {
                return opt.perNights ? days >= 1 ? days + ' ' + translate('nights') : '' : days > 1 ? days + ' ' + translate('days') : '';
            },
            showTopbar: true,
            swapTime: false,
        ...
        function dayClicked(day) {
            ...
            if (opt.singleDate) {
                ...
                if (opt.startOfWeek === 'monday') {
                    if(opt.weekOffset) {
                        opt.start = moment(parseInt(time)).startOf('isoweek').day(opt.weekOffset).valueOf();
                    } else {
                    ...
                    if(opt.weekOffset) {
                        opt.start = moment(parseInt(time)).startOf('week').day(opt.weekOffset).valueOf();
                    } else {
            ...
            if (opt.start && !opt.end && !opt.singleDate) {
                ...
                if (opt.beforeShowDay && typeof opt.beforeShowDay == 'function') {
                    ...
                    while (countDays(timeTmp, opt.start) > 1) {
                        ...
                        var disabledDay = window.JetPlugins.hooks.applyFilters( 'jet-booking.date-range-picker.disabled-day', !arr[0], arr, timeTmp );
                        if ( disabledDay ) {
                            valid = false;
                            break;
                        }
        ...
        function updateSelectableRange() {
            ...
            box.find('.day.toMonth.only-checkout.valid').removeClass('valid').addClass('invalid');
            ...
            if (opt.start && !opt.end) {
                box.find('.day.toMonth').each(function() {
                    ...
                    if ($(this).hasClass('valid') || $(this).hasClass('only-checkout')) {
                        if (!isValidTime(time))
        ...
        function setSeasonalOptions( opt ) {
            const jetABAFData = window.JetABAFData;
            const jetABAFConfig = window.JetABAFConfig;
            const seasonalPrice = jetABAFData ? jetABAFData.seasonal_price : jetABAFConfig.seasonal_price;
            const startDayOffset = jetABAFData ? +jetABAFData.start_day_offset : +jetABAFConfig.start_day_offset;
            const minDays = jetABAFData ? +jetABAFData.min_days : +jetABAFConfig.min_days;
            const maxDays = jetABAFData ? +jetABAFData.max_days : +jetABAFConfig.max_days;

            let rangeStart = opt.start ? Math.floor( opt.start / 1000 ) :Math.floor(Date.now() / 1000);
            ...
            if ( JSON.stringify( seasonalPrice ) !== JSON.stringify( {} ) ) {
                for ( let key in seasonalPrice ) {
                    if ( rangeStart >= +seasonalPrice[ key ].start && rangeStart <= +seasonalPrice[ key ].end ) {
                        rangeStartInSeason = true;

                        if ( seasonalPrice[ key ].enable_config ) {
                            opt.startDate = +seasonalPrice[ key ].start_day_offset ? moment().add( +seasonalPrice[ key ].start_day_offset, 'd' ) : new Date();
            ...
            if ( ! rangeStartInSeason ) {
                opt.startDate = startDayOffset ? moment().add( Number( startDayOffset ), 'd' ) : new Date();
                opt.minDays = minDays;
                opt.maxDays = maxDays;
            }
                    ...
                    if (opt.start && !opt.end) {
                        ...
                        if (opt.hoveringTooltip) {
                            if (typeof opt.hoveringTooltip == 'function') {
                                ...
                                tooltip = days + ' ' + opt.perNights ? translate('nights') : translate('days');
                            }
                        }
                    }
                }
        ...
        function checkSelectionValid() {
            var days = Math.ceil((opt.end - opt.start) / 86400000);
            ...
            if (opt.batchMode) {
                if (
                    ...
                    if (opt.excludedDates.length) {
                        for (var i = 0; i < days; i++) {
                            var day = opt.start + 86400000 * i;

                            if (day < opt.end && 0 <= opt.excludedDates.indexOf(moment(day).format('YYYY-MM-DD'))) {
                                opt.start = false;
                                opt.end = false;
                                box.find('.day').removeClass('checked');
                            }
        ...
        function countDays(start, end) {
            var daysCount = Math.abs(moment(start).diff(moment(end), 'd'));
            if(!opt.perNights) daysCount++;
            return daysCount;
        }
        ...
        function createDom() {
            ...
            var selectedDaysText = translate('days');

            if (opt.perNights) selectedDaysText = translate('nights');
            ...
            if (opt.showTopbar) {
                ...
                if (opt.customTopBar) {
                    ...
                    if (!opt.singleDate) {
                        html += ' <span class="separator-day">' + opt.separator + '</span> <b class="end-day">...</b> <i class="selected-days">(<span class="selected-days-num">3</span> ' + selectedDaysText + ')</i>';
                    }
                    html += '</div>';
