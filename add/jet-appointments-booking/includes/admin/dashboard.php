...
class Dashboard {
	...
	public function __construct( $pages = array() ) {
		...
		$this->components->appointments_range = new Helpers\Common_Component( 'appointments-range' );
	...
	public function assets( $hook ) {
		...
		$this->current_page->enqueue_script( 'jet-apb-dashboard-common', 'admin/common.js' );
		...
		if ( $config->is_set() ) {
			wp_localize_script( $config->get( 'handle' ), 'JetAPBConfig', $config->get( 'config' ) );
		}
	...
	public function register_pages() {
		...
		if ( $services_cpt ) {
			add_submenu_page(
				$parent,
				esc_html__( 'Services', 'jet-appointments-booking' ),
				esc_html__( 'Services', 'jet-appointments-booking' ),
				'manage_options',
				add_query_arg(
					array(
		...
		if ( $providers_cpt ) {
			add_submenu_page(
				$parent,
				esc_html__( 'Providers', 'jet-appointments-booking' ),
				esc_html__( 'Providers', 'jet-appointments-booking' ),
				'manage_options',
				add_query_arg(
					array(
						'post_type' => $providers_cpt,
					),
					admin_url( 'edit.php' )
				)
			);
		...
		add_submenu_page(
			$parent,
			esc_html__( 'Settings', 'jet-appointments-booking' ),
			esc_html__( 'Settings', 'jet-appointments-booking' ),
			'manage_options',
			add_query_arg(
				array(
					'page'    => 'jet-dashboard-settings-page',
					'subpage' => 'jet-apb-general-settings'
				),
				admin_url( 'admin.php' )
			)
		);
		...
		if ( ! Plugin::instance()->settings->get( 'hide_setup' ) ) {
			add_submenu_page(
				$parent,
				esc_html__( 'Set Up', 'jet-appointments-booking' ),
				esc_html__( 'Set Up', 'jet-appointments-booking' ),
				'manage_options',
				add_query_arg(
					array(
						'page'    => 'jet-dashboard-settings-page',
						'subpage' => 'jet-apb-set-up'
					),
					admin_url( 'admin.php' )
				)
			);
