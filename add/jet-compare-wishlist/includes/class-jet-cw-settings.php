...
if ( ! class_exists( 'Jet_CW_Settings' ) ) {
	...
	class Jet_CW_Settings {
		 ...
		 *
		 * @since 1.1.0
		 * @since 1.5.2 `compare_message_max_items` setting added.
		 *
		 * @return array
		 */
		public function get_localize_data() {
			...
			return array(
				...
				'settingsData'   => array(
					...
					'compare_message_max_items'   => [
						'value' => $this->get( 'compare_message_max_items', 'You can`t add more product in compare' ),
					],
		 ...
		 * Get.
		 *
		 * Returns dashboard setting my key name.
		 *
		 * @since  1.0.0
		 * @access public
		 *
		 * @param string  $setting Setting key name.
		 * @param boolean $default Default settings.
		 *
		 * @return bool|mixed
		 */
		public function get( $setting = '', $default = false ) {
			...
			return apply_filters( 'jet-cw/dashboard/settings/' . $setting, $this->settings[ $setting ] ?? $default, $this );
